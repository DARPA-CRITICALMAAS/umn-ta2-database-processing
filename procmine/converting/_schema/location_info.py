...
def sch_location_info(pl_data: pl.DataFrame,
    ...
    pl_loc_data.to_pandas().to_csv('/home/yaoyi/pyo00005/CriticalMAAS/outputs/tmp1.csv')
    ...
    list_count_stat = list(set(list_location_info) & {'country', 'state_or_province'})
    pl_loc_data = pl.concat(
        [pl_loc_data, pl_mapped_loc],
        how='align'
    ).with_columns(
        pl.col(list_count_stat).list.first()
    )
    ...
    try: 
        pl_loc_data = pl_loc_data.rename({'epsg': 'crs'}).with_columns(
            pl.col('crs').list.first()
        )
    except: pass
